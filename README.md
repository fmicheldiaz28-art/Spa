...
| Mensajes a clientas editables (recordatorios y avisos de lista de espera, por email y WhatsApp) con variables, vista previa y restaurar | ✅ | `/app/configuracion/mensajes` |
| Portal de la clienta: sus datos y preferencias (recordatorios sí/no, promociones con consentimiento registrado) | ✅ | `/mi-cuenta` |
| Recordatorios automáticos y OTP por WhatsApp (API de WhatsApp Business) | ⏳ Requiere la aprobación de Meta | docs/10 §17 |
| RLS en PostgreSQL | ⏳ Junto con el modo multi-empresa (Fase 3): con una sola organización, el aislamiento ya lo aplica el API en cada consulta | docs/11 §19 |
...
- **Base de desarrollo (PGlite) estable:** `pglite-socket` 0.1.x no liberaba las conexiones cortadas de golpe (cada reinicio del API en modo desarrollo) y tras 4 cortes rechazaba todo. `infra/scripts/dev-db-lite.mjs` purga esas conexiones; verificado con 8 cortes seguidos a mitad de transacción.
